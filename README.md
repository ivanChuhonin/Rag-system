...
RAG-система над постами ВКонтакте: **собрал контент → нашёл релевантные фрагменты → получил ответ со ссылками
на источники**. Источник данных — 5 VK-пабликов про кино (38k+ постов), поиск гибридный (BM25 + FAISS),
генерация — через Ollama (облачная `gemma4:cloud`) или GigaChat, простой чат-UI поверх FastAPI.
...
flowchart LR
    ...
    FAISS --> API
    DB --> API[api.py\nFastAPI]
    API -->|LLM| Ollama[Ollama / GigaChat]
...
- **Сбор**: `wall.get` VK API постранично по всей истории стены (не упирается в потолок листинга сайта, как
  было с первой версией на sports.ru) — дедуп по `url` и `sha256(текст)`, инкрементальный докач новых пабликов
  без перекачки уже собранных (`--skip-existing`).
- **Очистка текста**: разметка ВК (`[url|подпись]`), голые ссылки, `#` у хэштегов — вырезаются перед сохранением.
...
| LLM | Ollama (облачные модели, напр. `gemma4:cloud`) или GigaChat API |
...
нет с коротким обоснованием — так метрику можно выборочно перепроверить по JSON-выгрузке, а не просто поверить
на слово.
